...
public class ListPhpAdapter extends PhpCollectionPhpAdapter
{
	@Override
	public VisitResult		processMethodInvocation(VisitResult path, Method method, List<VisitResult> arguments, MethodInvocationTree node, Trees trees)
	{
		...
		if (method.getName().equals("add") && node.getArguments().size() == 2) {
			sb.append("->addIndexed(");
			appendArgumentsDirect(sb, node.getArguments(), trees);
			sb.append(")");
			return path.appendSb(sb);
		}
		if (method.getName().equals("addAll") && node.getArguments().size() == 2) {
			sb.append("->addAllIndexed(");
			appendArgumentsDirect(sb, node.getArguments(), trees);
			sb.append(")");
			return path.appendSb(sb);
		}
