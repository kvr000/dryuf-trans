 ...
 */
package org.druf.trans.php.adapter.com.google.common.collect;
...
public class SetsPhpAdapter extends BasicPhpAdapter
{
	public VisitResult		processMethodInvocation(VisitResult path, Method method, List<VisitResult> arguments, MethodInvocationTree node, Trees trees)
	{
		...
		if (method.getName().equals("filter") && node.getArguments().size() == 2) {
			sb.append("\\org\\druf\\util\\Sets::filter(");
			appendArgumentsDirect(sb, node.getArguments(), trees);
			sb.append(")");
			return new VisitResult(sb.toString());
		}
		else if (method.getName().equals("newLinkedHashSet") && node.getArguments().size() == 1) {
			sb.append("\\org\\druf\\util\\LinkedHashSet::createFromArray(");
			appendArgumentsDirect(sb, node.getArguments(), trees);
			sb.append(")");
			return new VisitResult(sb.toString());
		}
