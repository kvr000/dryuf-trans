...
import org.druf.trans.php.adapter.WrongPhpAdapter;


public class DatePhpAdapter extends WrongPhpAdapter
{
	@Override
	public VisitResult		processMethodInvocation(VisitResult path, Method method, List<VisitResult> arguments, MethodInvocationTree node, Trees trees)
	{
		...
		if (method.getName().equals("UTC") && method.getParameterTypes().length == 6) {
			sb.append("(gmmktime(");
			sb.append(arguments.get(3).getContent()).append(", ").append(arguments.get(4).getContent()).append(", ").append(arguments.get(5).getContent()).append(", ");
			sb.append(arguments.get(1).getContent()).append("+1, ").append(arguments.get(2).getContent()).append(", ").append(arguments.get(0).getContent());
			sb.append(")*1000)");
