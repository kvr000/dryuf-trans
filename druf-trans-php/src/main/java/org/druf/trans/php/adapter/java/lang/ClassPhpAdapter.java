...
public class ClassPhpAdapter extends BasicPhpAdapter
{
	...
	public VisitResult		processMethodInvocation(VisitResult path, Method method, List<VisitResult> arguments, MethodInvocationTree node, Trees trees)
	{
		...
		if (method.getName().equals("getName")) {
			...
			return new VisitResult(sb.append("\\org\\druf\\core\\Druf::createClassArg0(").append(path.getContent()).append(")"));
		}
		else if (method.getName().equals("getAnnotation")) {
			String[] m;
			return new VisitResult(sb.append("\\org\\druf\\core\\Druf::getClassAnnotation(").append(path.getContent()).append(", ").append(arguments.get(0).getFinal()).append(")"))
