 ...
 */
package net.dryuf.trans.php.adapter.org.apache.commons.codec.binary;
...
public class Base64PhpAdapter extends BasicPhpAdapter
{
	@Override
	public VisitResult		processMethodInvocation(VisitResult path, Method method, List<VisitResult> arguments, MethodInvocationTree node, Trees trees)
	{
		...
		if ((method.getName().equals("encodeBase64") || method.getName().equals("encodeBase64String")) && method.getParameterTypes().length == 1) {
			sb.append("base64_encode(");
			appendArgumentsDirect(sb, node.getArguments(), trees);
			sb.append(")");
			return new VisitResult(sb.toString());
		}
		else if (method.getName().equals("decodeBase64") && method.getParameterTypes().length == 1) {
			sb.append("base64_decode(str_replace('-', '+', str_replace('_', '/', ");
		appendArgumentsDirect(sb, node.getArguments(), trees);
			sb.append(")))");
			return new VisitResult(sb.toString());
		}
		else if ((method.getName().equals("encodeBase64URLSafe") || method.getName().equals("encodeBase64URLSafeString")) && method.getParameterTypes().length == 1) {
			sb.append("str_replace('+', '-', str_replace('/', '_', base64_encode(");
			appendArgumentsDirect(sb, node.getArguments(), trees);
			sb.append(")))");
			return new VisitResult(sb.toString());
		}
