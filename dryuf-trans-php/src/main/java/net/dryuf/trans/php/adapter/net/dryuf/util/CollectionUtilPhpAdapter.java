...
public class CollectionUtilPhpAdapter extends BasicPhpAdapter
{
	@Override
	public VisitResult		processMethodInvocation(VisitResult path, Method method, List<VisitResult> arguments, MethodInvocationTree node, Trees trees)
	{
		...
		if (method.getName().equals("createHashSet")) {
			if (arguments.get(0).getResultClass() == String.class) {
				sb.append("\\net\\dryuf\\util\\CollectionUtil::createStringNativeHashSet");
			}
			else if (visitor.isClassIntegerish(arguments.get(0).getResultClass())) {
				sb.append("\\net\\dryuf\\util\\CollectionUtil::createNativeHashSet");
			}
			else {
				sb.append("\\net\\dryuf\\util\\CollectionUtil::createHashSet");
			}
			sb.append("(");
